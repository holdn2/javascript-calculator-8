...
describe('validateRawInput', () => {
  ...
  test('커스텀 지시자가 모두 포함된 경우 // 가 맨 앞에 있지 않으면 에러를 발생시킨다.', () => {
    ...
    expect(() => {
      validateRawInput(input);
    }).toThrow(ERROR_MESSAGE.INVALID_CUSTOM_SEPARATOR_START);
  });
...
describe('validateCustomSeparator', () => {
  test('커스텀 구분자의 길이가 1보다 크다면 에러를 발생시킨다.', () => {
    ...
    }).toThrow(ERROR_MESSAGE.INVALID_SEPARATOR_LENGTH);
  });
  ...
  test('커스텀 구분자가 숫자라면 에러를 발생시킨다.', () => {
    const customSeparator = '6';

    expect(() => {
      validateCustomSeparator(customSeparator);
    }).toThrow(ERROR_MESSAGE.INVALID_CUSTOM_SEPARATOR_NUMBER);
  });
  ...
  test('커스텀 구분자가 기본 구분자(,)와 같다면 에러를 발생시킨다.', () => {
    ...
    expect(() => {
      validateCustomSeparator(customSeparator);
    }).toThrow(ERROR_MESSAGE.INVALID_CUSTOM_SEPARATOR_DEFAULT);
  });
...
describe('validateParsedInput', () => {
  ...
  test('구분자가 연속으로 오면 에러를 발생시킨다.', () => {
    ...
    expect(() => {
      validateParsedInput(escapedSeparator, parsedInput);
    }).toThrow(ERROR_MESSAGE.MISSING_NUMBER_BETWEEN_SEPARATORS);
  });
